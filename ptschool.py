...
def sign() :
    ...
    pattern = re.compile('<a\s*href="mybonus.php">使用</a>]: ([0-9,\.]+) <font')
...
def main() :
    ...
    global BASEURL
    BASEURL = 'https://pt.btschool.club/index.php'

    global SIGNURL
    SIGNURL = 'https://pt.btschool.club/index.php?action=addbonus'

    global HTTP
    HTTP = urllib3.PoolManager()

    global PRIVACY
    PRIVACY =  os.getenv('SCHOOL_PRIVACY') if os.getenv('SCHOOL_PRIVACY') else '1'

    _uid = os.getenv('SCHOOL_UID') if os.getenv('SCHOOL_UID') else False
    _pass = os.getenv('SCHOOL_PASS') if os.getenv('SCHOOL_PASS') else False
    _login = os.getenv('SCHOOL_LOGIN') if os.getenv('SCHOOL_LOGIN') else 'bm9wZQ%3D%3D'
    _ssl = os.getenv('SCHOOL_SSL') if os.getenv('SCHOOL_SSL') else 'eWVhaA%3D%3D'
    _tracker_ssl = os.getenv('SCHOOL_TRACKER_SSL') if os.getenv('SCHOOL_TRACKER_SSL') else 'eWVhaA%3D%3D'
    _multi =  os.getenv('SCHOOL_MULTI') if os.getenv('SCHOOL_MULTI') else False
